    ...
    if st.session_state.search_results:
        ...
        for case in st.session_state.search_results:
            with st.expander(f"{case['id']} - {case['title']}", expanded=True):
                ...
                # Add action buttons for the case with improved styling - icon-only with hover text
                st.markdown("""
                <style>
                .action-button {
                    display: inline-flex;
                ...
                <div class="action-buttons-container">
                    <a href="{pdf_url}" target="_blank" class="action-button" data-tooltip="View Full Case PDF">
                    ...
                    <button onclick="navigator.clipboard.writeText('{citation}'); const msg = document.createElement('div'); msg.className = 'success-message'; msg.innerText = 'Citation copied!'; this.appendChild(msg); setTimeout(() => msg.remove(), 2000);" class="action-button" data-tooltip="Copy Citation">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <button onclick="const paragraphs = document.querySelectorAll('.relevant-paragraph'); let text = ''; paragraphs.forEach(p => text += p.innerText + '\\n\\n'); navigator.clipboard.writeText(text); const msg = document.createElement('div'); msg.className = 'success-message'; msg.innerText = 'Passages copied!'; this.appendChild(msg); setTimeout(() => msg.remove(), 2000);" class="action-button" data-tooltip="Copy Relevant Passages">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
